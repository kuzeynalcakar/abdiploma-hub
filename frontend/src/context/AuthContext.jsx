...
export function AuthProvider({ children }) {
  ...
  // Until the stored token is checked we don't know if the user is
  // logged in, so protected routes wait instead of redirecting.
  const [isLoading, setIsLoading] = useState(hasSessionHint())
  // Transient network failures while restoring a session (token kept).
  ...
  const restoreSession = useCallback(async () => {
    ...
    try {
      ...
      if (err?.status === 401) {
        ...
        // Keep the token on network / 5xx so a retry can succeed.
        setUser(null)
        setSessionError(
