"""Select ~300 production Chemistry 30 questions from the validated pool.
...
Output: chemistry30_questions_final.json and course_questions_final.json
"""
...
INPUT = HERE / "chemistry30_questions_pool.json"
OUTPUT = HERE / "chemistry30_questions_final.json"
...
# Diploma unit weighting (21 / 29 / 21 / 29), adjusted to pool availability.
# All NR in the pool are retained (99); MC fills the remainder (201).
TOPIC_TYPE_TARGETS = {
    "Thermochemical Changes": {"Multiple Choice": 46, "Numerical Response": 24},
    "Electrochemical Changes": {"Multiple Choice": 49, "Numerical Response": 34},
...
TOPIC_TARGETS = {
    topic: sum(targets.values())
    for topic, targets in TOPIC_TYPE_TARGETS.items()
}

TYPE_TARGETS = {
    "Multiple Choice": 201,
    "Numerical Response": 99,
}
...
MAX_PER_OUTCOME_MC = 12
MAX_PER_OUTCOME_NR = 20
...
def score_item(
    ...
    *,
    ...
    score += outcome_need[q["outcome_code"]] * 6
    score += cognitive_need[cognitive_bucket(q)] * 2
    ...
    score -= template_counts[tmpl] * 2
    score -= skill_counts[q["skill_tested"]] * 0.5
...
def pick_for_slot(
    ...
    *,
    ...
    while len(chosen) < need and remaining:
        ...
        cognitive_need = Counter({"knowledge": 2, "sts": 2, "skills": 2})
...
def main() -> None:
    if not INPUT.is_file():
        print(f"Input not found: {INPUT}. Run generate_chemistry30.py first.")
        raise SystemExit(1)
