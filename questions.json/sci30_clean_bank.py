"""Select ~300 production Science 30 questions from the validated pool.

Optimizes curriculum coverage, diploma GO weighting, outcome balance,
cognitive diversity, difficulty mix, topic balance, and minimum template
repetition.

The pool contains only 89 Numerical Response items (20% of pool), so the
final bank retains all 89 NR and fills to 300 with Multiple Choice
(70.3% MC / 29.7% NR). Ideal 60/40 split would require 120 NR; pool
capacity constrains NR to 89 (adjusted per curriculum).

Output: science30_questions_final.json and course_questions_final.json
"""
...
INPUT = HERE / "science30_questions_pool.json"
OUTPUT = HERE / "science30_questions_final.json"
...
# Diploma GO weighting (blueprint TOPIC_TYPE_TARGETS), adjusted so every
# pool NR is retained and topic totals remain 38/38/75/46/29/74.
TOPIC_TYPE_TARGETS = {
...
def rebalance_cognitive(
    ...
    *,
    ...
    """Maximize STS/skills coverage by swapping knowledge MC within each topic."""
    selected_set = {id(q) for q in selected}
    ...
    for bucket in ("sts", "skills"):
        ...
        for topic in TOPIC_TARGETS:
            topic_pool = [q for q in pool_bucket if q["topic"] == topic and id(q) not in selected_set]
...
def print_report(items: list[dict]) -> None:
    print("\n=== Science 30 Production Bank ===")
    print(f"Total: {len(items)}")
    print("\nTopic distribution (diploma GO weighting):")
...
def main() -> None:
    if not INPUT.is_file():
        print(f"Input not found: {INPUT}. Run generate_science30.py first.")
        raise SystemExit(1)
