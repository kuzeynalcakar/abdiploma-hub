...
class AuthSecurityTests(unittest.TestCase):
    ...
    def test_normal_login_flow_and_cookie(self):

        email = _unique_email("login")

        self._register(email=email, password="LoginPass9")

        # Clear jar so login must set a fresh cookie
        ...
        self.assertEqual(response.cookies[cookie_name], token)



        # Cookie auth without Authorization header
        ...
        # Bearer auth still works for scripts
