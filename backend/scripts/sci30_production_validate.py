"""Validate and auto-fix the Science 30 production question bank.

Targets course_questions_final.json and science30_questions_final.json.
...
ALIAS = Path(__file__).parent.parent.parent / "questions.json" / "science30_questions_final.json"
REPORT = Path(__file__).parent.parent.parent / "questions.json" / "sci30_production_validation_report.json"
...
def audit_production_bank(pool: list[dict]) -> dict:
    ...
    for i, q in enumerate(pool):
        ...
        if q.get("course_code") != "SCI30":
            report["schema_errors"].append({"index": i, "reasons": ["course_code must be SCI30"]})
...
def fix_pool(pool: list[dict], report: dict) -> dict:
    ...
    # Second pass for explanations still duplicated after rewrite
...
def validate_every_question(pool: list[dict]) -> list[tuple[int, list[str]]]:
    ...
    for i, q in enumerate(pool):
        ...
        if q["question_type"] == "Multiple Choice":
            ...
            sig = distractor_signature(q)
            if sig:
                # checked globally in audit; per-question only if already flagged
                pass
