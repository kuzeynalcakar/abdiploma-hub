"""Import SCI10 production bank with backup, safety checks, and verification."""
...
REPORT_JSON = BACKEND.parent / "questions.json" / "sci10_production_readiness_report.json"
REPORT_MD = BACKEND.parent / "questions.json" / "SCI10_PRODUCTION_READINESS_REPORT.md"
BASE_API = "http://127.0.0.1:8000/api/v1"

SCI10_TOPIC_TARGETS = {
    "Energy and Matter in Chemical Change": 90,
    "Energy Flow in Technological Systems": 85,
    "Cycling of Matter in Living Systems": 85,
    "Energy Flow in Global Systems": 40,
}
SCI10_TOTAL = 300
SCI10_MC = 218
SCI10_NR = 82
...
def backup_db() -> Path:
    ...
    dest = BACKUP_DIR / f"albertaprep_pre_sci10_import_{stamp}.db"
...
def snapshot_existing(conn: sqlite3.Connection) -> dict:
    ...
    snap = {
        ...
        "sci10_count": course_count(conn, "SCI10"),
    }
...
def verify_sci10_counts(conn: sqlite3.Connection) -> list[str]:
    issues = []
    total = course_count(conn, "SCI10")
    if total != SCI10_TOTAL:
        issues.append(f"SCI10 total {total} != {SCI10_TOTAL}")
    ...
    for name, count in rows:
        if count == 0:
            continue
        expected = SCI10_TOPIC_TARGETS.get(name)
        if expected is None:
            issues.append(f"Unexpected SCI10 topic with questions: {name} ({count})")
        elif count != expected:
    ...
    dupes = conn.execute(
        ...
        JOIN topics t ON q.topic_id = t.id
        JOIN courses c ON t.course_id = c.id
        WHERE c.code = 'SCI10' AND q.is_active = 1
        GROUP BY q.question_text
        HAVING n > 1
        """
    ).fetchall()
    if dupes:
        issues.append(f"{len(dupes)} duplicate SCI10 stems in DB")
    ...
    if type_counts.get("multiple_choice", 0) != SCI10_MC:
        issues.append(
            f"SCI10 MC count {type_counts.get('multiple_choice', 0)} != {SCI10_MC}"
        )
    if type_counts.get("numerical_response", 0) != SCI10_NR:
        issues.append(
            f"SCI10 NR count {type_counts.get('numerical_response', 0)} != {SCI10_NR}"
        )
...
def verify_api_endpoints(conn: sqlite3.Connection) -> tuple[list[str], dict]:
    ...
    details: dict = {}
    try:
        email = f"sci10.import.{random.randint(100000, 999999)}@example.com"
        token = api_request(
            "/auth/register",
            method="POST",
        ...
        if sci is None:
            issues.append("SCI10 not listed in /courses")
            return issues, details

        details["course_question_count"] = sci.get("question_count", 0)
        if sci.get("question_count", 0) < SCI10_TOTAL:
            issues.append(
                f"/courses SCI10 question_count {sci.get('question_count')} < {SCI10_TOTAL}"
            )
        ...
        sample_mc = conn.execute(
            ...
            WHERE c.code = 'SCI10' AND q.question_type = 'multiple_choice'
            LIMIT 1
            """
        ).fetchone()
        if sample_mc:
        ...
        sample_nr = conn.execute(
            ...
            JOIN topics t ON q.topic_id = t.id
            JOIN courses c ON t.course_id = c.id
            WHERE c.code = 'SCI10' AND q.question_type = 'numerical_response'
            LIMIT 1
            """
        ).fetchone()
        if sample_nr:
...
def build_report(
    *,
    ...
    return {
        ...
        "import_summary": {
            "sci10_before": before["sci10_count"],
            "sci10_after": after["sci10_count"],
            "imported_new": after["sci10_count"] - before["sci10_count"],
        },
        "data_preservation": {
        ...
        "sci10_counts": {
            "total": after["sci10_count"],
            "target": SCI10_TOTAL,
            "by_topic": {name: count for name, count in topic_rows},
            "topic_targets": SCI10_TOPIC_TARGETS,
            "by_type": dict(type_rows),
            "type_targets": {"multiple_choice": SCI10_MC, "numerical_response": SCI10_NR},
        },
...
def write_markdown_report(report: dict) -> None:
    ...
    lines = [
        ...
        f"| Questions imported | **{report['import_summary']['imported_new']}** new (total **{report['import_summary']['sci10_after']}**) |",
    ...
    lines.extend([
        ...
        "",
        f"**Total:** {report['sci10_counts']['total']} / {report['sci10_counts']['target']}",
        "",
        "| Topic | Count | Target |",
        "|-------|-------|--------|",
    ])
    for topic, target in report["sci10_counts"]["topic_targets"].items():
        count = report["sci10_counts"]["by_topic"].get(topic, 0)
    ...
    for qtype, target in report["sci10_counts"]["type_targets"].items():
        count = report["sci10_counts"]["by_type"].get(qtype, 0)
    ...
    lines.extend([
        ...
        f"| GET /courses (SCI10 listed) | {api.get('courses_listed', 'N/A')} |",
    ...
    if report["issues"]:
        ...
        lines.extend([
            ...
            "## Verdict",
            "",
            "**Science 10 is production-ready.** All 300 questions imported, existing user data preserved, "
...
def main() -> int:
    ...
    sci_items = [i for i in items if i.get("course_code") == "SCI10"]
    if len(sci_items) != SCI10_TOTAL:
        raise SystemExit(f"Expected {SCI10_TOTAL} SCI10 items, got {len(sci_items)}")
    ...
    print(f"Before import — SCI10: {before['sci10_count']}")
    for code in PRESERVE_COURSES:
        key = f"{code.lower().replace('-', '_')}_count"
    ...
    if before["sci10_count"] == 0:
        print("Importing SCI10 production bank...")
        run_import(items)
    elif before["sci10_count"] == SCI10_TOTAL:
        print("SCI10 already imported (300 questions). Re-running import for duplicate-skip check only.")
        run_import(items)
        conn = sqlite3.connect(PROD_DB)
        count = course_count(conn, "SCI10")
        conn.close()
        if count != SCI10_TOTAL:
            raise SystemExit("Duplicate import changed SCI10 question count")
        print("Duplicate import check: 0 new rows (all 300 skipped as expected)")
    else:
        raise SystemExit(
            f"Partial SCI10 import detected ({before['sci10_count']} questions). "
    ...
    issues.extend(verify_sci10_counts(conn))
    ...
    print("\n=== IMPORT RESULT ===")
    print(f"SCI10 count: {before['sci10_count']} -> {after['sci10_count']}")
    for code in PRESERVE_COURSES:
        key = f"{code.lower().replace('-', '_')}_count"
