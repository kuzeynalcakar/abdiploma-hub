"""Import MATH30-2 production bank with backup, safety checks, and verification."""
...
REPORT_PATH = BACKEND.parent / "questions.json" / "math30-2_production_readiness_report.json"
MD_REPORT_PATH = BACKEND.parent / "questions.json" / "MATH30-2_PRODUCTION_READINESS_REPORT.md"
BASE_API = "http://127.0.0.1:8000/api/v1"

MATH30_2_TOPIC_TARGETS = {
...
MATH30_2_MC = 64
MATH30_2_NR = 236
MATH30_2_TOPIC_COUNT = 7
PRESERVE_COURSES = ("BIO30", "MATH30-1", "MATH20-1", "CHEM30", "PHYS30")


def backup_db() -> Path:
    BACKUP_DIR.mkdir(exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    dest = BACKUP_DIR / f"albertaprep_pre_math30_2_import_{stamp}.db"
...
def snapshot_existing(conn: sqlite3.Connection) -> dict:
    ...
    return {
        ...
        "math30_2_count": course_count(conn, "MATH30-2"),
    }
...
def verify_math30_2_counts(conn: sqlite3.Connection) -> list[str]:
    issues = []
    total = course_count(conn, "MATH30-2")
    if total != MATH30_2_TOTAL:
        issues.append(f"MATH30-2 total {total} != {MATH30_2_TOTAL}")
    ...
    for name, count in rows:
        if count == 0:
            continue
        expected = MATH30_2_TOPIC_TARGETS.get(name)
        if expected is None:
            issues.append(f"Unexpected MATH30-2 topic with questions: {name} ({count})")
        elif count != expected:
    ...
    if dupes:
        issues.append(f"{len(dupes)} duplicate MATH30-2 stems in DB")
    ...
    if type_counts.get("multiple_choice", 0) != MATH30_2_MC:
        issues.append(
            f"MATH30-2 MC count {type_counts.get('multiple_choice', 0)} != {MATH30_2_MC}"
        )
    if type_counts.get("numerical_response", 0) != MATH30_2_NR:
        issues.append(
            f"MATH30-2 NR count {type_counts.get('numerical_response', 0)} != {MATH30_2_NR}"
        )
...
def verify_api_endpoints(conn: sqlite3.Connection) -> tuple[list[str], dict]:
    ...
    details: dict = {}
    try:
        email = f"math30_2.import.{random.randint(100000, 999999)}@example.com"
        token = api_request(
            "/auth/register",
            method="POST",
            data={"name": "Math30-2 Import Verify", "email": email, "password": "testpass123"},
        )["access_token"]

        courses = api_request("/courses", token=token)["courses"]
        math302 = next((c for c in courses if c["code"] == "MATH30-2"), None)
        details["courses_listed"] = math302 is not None
        if math302 is None:
            issues.append("MATH30-2 not listed in /courses")
            return issues, details

        details["course_question_count"] = math302.get("question_count", 0)
        if math302.get("question_count", 0) < MATH30_2_TOTAL:
            issues.append(
                f"/courses MATH30-2 question_count {math302.get('question_count')} < {MATH30_2_TOTAL}"
            )

        topics = api_request(f"/courses/{math302['id']}/topics", token=token)["topics"]
        details["topic_count"] = len(topics)
        if len(topics) != MATH30_2_TOPIC_COUNT:
            issues.append(
                f"/courses/{{id}}/topics returned {len(topics)} topics, expected {MATH30_2_TOPIC_COUNT}"
            )

        available = api_request(
            f"/quiz/available-count?course_id={math302['id']}", token=token
        )
        ...
        quiz = api_request(
            f"/quiz/questions?course_id={math302['id']}&count=10", token=token
        )
        ...
        daily = api_request(f"/daily-practice?course_id={math302['id']}", token=token)
        ...
        daily_start = api_request(
            f"/daily-practice/start?course_id={math302['id']}",
            method="POST",
            token=token,
        )
        ...
        weakness = api_request(f"/weakness-map?course_id={math302['id']}", token=token)
        ...
        elif len(weakness.get("topics", [])) != MATH30_2_TOPIC_COUNT:
            issues.append(
                f"/weakness-map returned {len(weakness.get('topics', []))} topics, "
                f"expected {MATH30_2_TOPIC_COUNT}"
            )
        ...
        sample_mc = conn.execute(
            ...
            WHERE c.code = 'MATH30-2' AND q.question_type = 'multiple_choice'
            LIMIT 1
            """
        ).fetchone()
        if sample_mc:
        ...
        sample_nr = conn.execute(
            ...
            JOIN topics t ON q.topic_id = t.id
            JOIN courses c ON t.course_id = c.id
            WHERE c.code = 'MATH30-2' AND q.question_type = 'numerical_response'
            LIMIT 1
            """
        ).fetchone()
        if sample_nr:
...
def build_report(
    *,
    ...
    return {
        ...
        "import_summary": {
            "math30_2_before": before["math30_2_count"],
            "math30_2_after": after["math30_2_count"],
            "imported_new": after["math30_2_count"] - before["math30_2_count"],
        },
        "data_preservation": {
            ...
            "math20_1_count": {"before": before["math20_1_count"], "after": after["math20_1_count"]},
        ...
        "math30_2_counts": {
            "total": after["math30_2_count"],
            "target": MATH30_2_TOTAL,
            "by_topic": {name: count for name, count in topic_rows},
            "topic_targets": MATH30_2_TOPIC_TARGETS,
            "by_type": {name: count for name, count in type_rows},
            "type_targets": {"multiple_choice": MATH30_2_MC, "numerical_response": MATH30_2_NR},
        },
...
def write_md_report(report: dict) -> None:
    ...
    lines = [
        ...
        f"| MATH30-2 total after import | **{report['import_summary']['math30_2_after']}** |",
        ...
        f"| MATH20-1 questions | {ds['math20_1_count']['before']} | {ds['math20_1_count']['after']} | {'Yes' if ds['math20_1_count']['before'] == ds['math20_1_count']['after'] else 'No'} |",
        ...
        "## Course & Topic Counts",
        "",
        f"### MATH30-2 total: {mc['total']} (target {mc['target']})",
        "",
        "| Topic | Count | Target |",
        "|-------|-------|--------|",
    ]
    for topic, target in MATH30_2_TOPIC_TARGETS.items():
    ...
    lines.extend([
        ...
        "| Type | Count | Target |",
        "|------|-------|--------|",
        f"| Multiple Choice | {mc['by_type'].get('multiple_choice', 0)} | {MATH30_2_MC} |",
        f"| Numerical Response | {mc['by_type'].get('numerical_response', 0)} | {MATH30_2_NR} |",
        "",
        "---",
        "",
        "## API Verification",
    ...
    if api.get("api_available"):
        lines.extend([
            "| Endpoint | Result |",
            "|----------|--------|",
            f"| `GET /courses` | MATH30-2 listed, count={api.get('course_question_count')} |",
...
def main() -> int:
    ...
    math_items = [i for i in items if i.get("course_code") == "MATH30-2"]
    if len(math_items) != MATH30_2_TOTAL:
        raise SystemExit(f"Expected {MATH30_2_TOTAL} MATH30-2 items, got {len(math_items)}")
    ...
    print(
        ...
        f"MATH20-1: {before['math20_1_count']}, CHEM30: {before['chem30_count']}, "
        f"PHYS30: {before['phys30_count']}, MATH30-2: {before['math30_2_count']}"
    )
    print(
    ...
    if before["math30_2_count"] == 0:
        print("Importing MATH30-2 production bank...")
        run_import(items)
    elif before["math30_2_count"] == MATH30_2_TOTAL:
        print("MATH30-2 already imported (300 questions). Re-running import for duplicate-skip check only.")
        run_import(items)
        conn = sqlite3.connect(PROD_DB)
        count = course_count(conn, "MATH30-2")
        conn.close()
        if count != MATH30_2_TOTAL:
            raise SystemExit("Duplicate import changed MATH30-2 question count")
        print("Duplicate import check: 0 new rows (all 300 skipped as expected)")
    else:
        raise SystemExit(
            f"Partial MATH30-2 import detected ({before['math30_2_count']} questions). "
    ...
    issues.extend(verify_math30_2_counts(conn))
    ...
    print("\n=== IMPORT RESULT ===")
    print(f"MATH30-2 count: {before['math30_2_count']} -> {after['math30_2_count']}")
